...
rawcruisedata<-read.csv("BAY873.csv")
...
DOidpattern<-glob2rx(paste("DO*",thisyear,"_",thismonth,"full",earlyorlate,".txt",sep=""))
DOdata<-list.files(pattern=DOidpattern)
DOdata<-read.csv(DOdata)

Wtempidpattern<-glob2rx(paste("WTEMP*",thisyear,"_",thismonth,"full",earlyorlate,".txt",sep=""))
...
surfacedataplotly<-plot_ly()%>%
  ...
  config(toImageButtonOptions = list(format= 'png', # one of png, svg, jpeg, webp
                                     filename= paste(earlyorlate,thiscruise,'Striped Bass Habitat - Fishing Areas',sep=" "),
                                     height= 500,
                                     width= 700,
                                     scale= 1 ))
...
surfacedataplotly
htmlwidgets::saveWidget(as_widget(surfacedataplotly), paste("D:/Desktop/Habitat Plots/MDbaysurface",earlyorlate,thiscruise,thisyear,Sys.Date(),".html"))
